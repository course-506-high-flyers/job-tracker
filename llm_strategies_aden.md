...
| 6 | WAF / access-log mining as a passive signal | Not applicable yet | The deployed host now sees internet-wide scanner noise, but we have not built a mining pipeline. The strategy is named here so a future-me reading this file knows the option exists once we accumulate enough real access logs to baseline normal traffic. |
