...
- **Steps 6-8:** PASS - Live browser testing on the team EC2 confirmed the application routes are now available. GitHub OAuth login succeeded on `https://35.86.191.136`, the authenticated user could create and view an application at `/applications/<id>`, and the application detail page rendered only the logged-in user's record.
